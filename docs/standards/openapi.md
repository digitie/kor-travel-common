...
5. 계층 3(S1~S13) 미채택은 예외가 아니라 "미채택"이며 소비자 매니페스트에만 기록한다. 단 geo v2처럼 타 저장소가 직접 소비하거나 concierge features export처럼 변경이 타 저장소를 깨뜨리는 **외부 계약**은 구체적인 `surface`로 등록할 수 있다. 이때 `reason`에는 긍정형 `소비하는 외부 계약` 또는 `소비되는 외부 계약`, 정합 task ID, `M10` 또는 동반 PR 근거를 모두 적고 부정 표현은 허용하지 않는다. `surface`는 `*`·`/*`·`/**` 같은 전역 wildcard가 아닌 구체 표면이어야 하며 validator가 이를 강제한다.
