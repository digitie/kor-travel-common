...
5. 계층 3(S1~S13) 미채택은 예외가 아니라 "미채택"이며 소비자 매니페스트에만 기록한다. 단 geo v2처럼 타 저장소가 직접 소비하거나 concierge features export처럼 변경이 타 저장소를 깨뜨리는 **외부 계약**은 구체적인 `surface`로 등록할 수 있다. 이때 `reason`에는 문장 중간 substring이 아닌 정확히 닫힌 긍정 assertion `소비하는 외부 계약이다.`·`소비되는 외부 계약이다.`·`임.`을 적고, 정합 task ID와 독립된 `M10` 또는 `동반 PR` token을 적는다. 여는 괄호·질문형·불확정 문장은 assertion으로 인정하지 않는다. task ID는 파일명에 있는 `T-NNN[a-z]?`의 standalone token이어야 하며 `_`·한국어 문자·`.1`·`-suffix`·결합문자를 붙이지 않는다. `M10`과 `동반 PR`도 identifier·결합문자·점·underscore 접미사를 허용하지 않는다. 부정·불확정 근거와 예약 YAML indicator는 허용하지 않는다. `surface`는 `*`·`/*`·`/**` 같은 전역 wildcard가 아닌 구체 표면이어야 하며 validator가 이를 강제한다.
