...
| `consumer-smoke` | `workflow_dispatch` + 주간 | `consumers.pins.json`(role·url·revision, ktdm runtime pin 형식) pinned SHA로 map admin·pinvi web 체크아웃 → tarball 설치 → type-check + `next build` | ubuntu-24.04 | T-010 |
