...
- `poetry.lock` 검사기는 `[[package]]`의 `name`·`version`·유효한 source 형식을 축과 대조하고 `[package.source] type = "git"`의 `reference`·`resolved_reference`를 검사한다. `metadata.python-versions`가 없거나 문자열·범위가 아니면 입력 오류(exit 2)로 닫는다. 40자리 `resolved_reference`는 lock SHA로 확인하지만 선언 branch는 `FLOATING_REF`로 남긴다. `requirements*.txt`의 `-r`·`--requirement`(공백·등호·축약형)는 상대 파일을 재귀 확장하고 순환·누락·미지원 옵션·형식 오류는 exit 2로 닫는다. 인라인 주석과 per-requirement `--hash`는 선언에서 제거한 뒤 검사하며 editable Git도 같은 고정 참조 규칙을 따른다. `==2.*`·`~=`·교집합 등 지원하는 PEP 440 범위가 `blocked[]`와 겹치면 설치본이 없어도 `BLOCKED`로 보고한다.
