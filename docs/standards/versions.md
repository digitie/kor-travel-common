...
- Python: `uv.lock` 커밋 + CI·Docker 모두 `uv sync --locked`(weather 선례, `vm` §2.1). airport(CI만)·pinvi(미소비)는 T-482·T-484에서 소비 일관화한다. 과도기 Poetry는 `poetry.lock`의 package/version과 git source를 제한적으로 읽고, `requirements.txt`는 재귀 선언을 읽되 정확 `==` 핀만 설치본 후보로 삼는다. requirements 결과에는 `NO_LOCK`을 남기며 범위 선언은 설치본 대조가 아니다. Poetry parser의 결과도 uv lock 도입·소비자 gate를 대신하지 않는다(T-005b, T-471·T-450).
- `poetry.lock` 검사기는 `[[package]]`의 `name`·`version`을 축과 대조하고 `[package.source] type = "git"`의 `reference`·`resolved_reference`를 검사한다. 40자리 `resolved_reference`는 lock SHA로 확인하지만 선언 branch는 `FLOATING_REF`로 남긴다. `requirements.txt`의 `-r`·`--requirement`는 상대 파일을 재귀 확장하고 순환·누락·형식 오류는 exit 2로 닫는다. `mcp`처럼 `blocked[]`와 선언 범위가 겹치면 설치본이 없어도 `BLOCKED`로 보고한다.
...
- 한계(사실): Poetry lock은 package/version·git source만 제한적으로 읽고 requirements 설치본은 정확 핀 후보와 차단 범위만 보고하며 두 입력 모두 uv lock을 대신하지 않는다. Docker 이미지·GitHub Actions·CI Node 버전은 읽지 않으며 workflow 정적 참조·Node 선언은 T-005c 미구현으로 NOT_RUN이다. 실제 실행 버전·Docker 파서는 범위 밖이고 git 고정 판정은 §3.8 휴리스틱이다. npm 사전 배포 버전·별칭·로컬 링크는 안정 원 패키지와 비교하지 않고 `NO_LOCK`으로 보고한다. npm 설치본은 `major.minor.patch`와 선택적 build metadata만 지원한다. 런타임/정책 숫자 접두와 이미지 `-slim`은 별도 허용한다. 런타임 하한은 단일 숫자/`=`/`==`/`^`/`~`/`~=`/`>=` 접두, `>=`·`<` 교집합, 이들의 `||` 대안을 지원한다. 빈 교집합과 `>`·`<=`·`!=` 및 그 밖의 혼합 문법은 NO_ENGINES다. 전체 SemVer/PEP 440 범위 해석기가 아니며 lock과 선언의 만족 여부는 소비자의 `npm ci`·`uv sync --locked` gate가 검증한다.
