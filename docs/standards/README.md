...
규칙 문서는 패키지·도구·워크플로와 대조해 확정한다. design-tokens.md는 T-104에서 두 적대적 리뷰를 통과해 확정했다. 나머지 문서는 각 표의 확정 task가 끝날 때까지 정본 초안으로 관리한다. 확정 전에도 신규 코드는 해당 문서를 따르고 실물과 어긋나면 같은 리뷰 gate에서 문서를 고친다.

## 2. 문서 목록

| 문서 | 한 줄 | 적용 대상 | 검증 수단 | 규칙 ID | 확정 task |
|---|---|---|---|---|---|
| [design-tokens.md](design-tokens.md) | `--kt-*` 토큰 계약: 정본 CSS·계층·역할·shadcn alias·프로필·오버라이드·다크·대비·값 형식·폰트·별칭 shim | tokens 소비 앱 전부(admin 6 + pinvi admin; consumer 프로필은 의미 이름만) | `tools/kt_contrast.py`(report + `contrast-baseline.json`), `packages/tokens` 생성물 diff | `TK-n` | T-104 완료(선행 T-101) |
