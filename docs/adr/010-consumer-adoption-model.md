...
1. 첫 소비자·순서(D-16): L6를 Phase 0 외부 확인(T-020)으로 승격. tokens 1차 = map + weather(즉시) + pinvi admin(L6 완료 조건) + airport(WIP 병합 후). ui 1차 = map + pinvi admin(L6); L6가 T-2xx 착수까지 미결이면 airport 소형 부품으로 대체. Python 1차 = map-api·weather-api·airport, 2차 geo, 3차 pinvi·concierge·ktdm(L8 후, breaking 묶음). concierge·ktdm은 L8 전에는 규칙 문서·`tokens.json` 참조까지만.
...
8. 소비자 저장소는 직접 수정하지 않는다. 외부 선행(L6·L8·WIP 병합·React 19)은 결정 기록과 PR 요청 문서(T-020·T-021·T-505)로 요청한다.
...
- pinvi·ktc·ktdm 트랙은 사용자 결정(O-1·O-2)에 종속되며 `adoption-readiness.md`에 `WAIT`로 남는다.
