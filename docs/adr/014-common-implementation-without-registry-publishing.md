...
3. common 자체 구현·검증을 마친 0.1 후보를 각각 T-109a(tokens), T-212a(UI), T-310a(Python)에서 보존한다. 이 하위 task가 DONE이면 다음 common 구현을 시작할 수 있다. T-201은 T-109a, T-205는 T-212a, T-306·T-307은 T-310a를 선행으로 한다. 이전 정식 릴리스는 구현 선행에서 제외한다. UI 0.2 common 후보 보존은 T-213a, Python 0.2 후보 보존은 T-311이 소유하며, 외부 UI rc·정식 릴리스는 T-213이 T-213a 후보와 T-212 외부 evidence를 사용해 진행한다. 각 task가 검증한 해당 minor 전체 소스에서 새 후보를 만들며 0.1 후보의 버전만 0.2로 바꾸지 않는다.
...
[패키지 계약](../architecture/packages.md), [릴리스 절차](../runbooks/release.md), [통합 계획](../plan/integration-plan.md), T-006·T-010/010a·T-109/109a·T-201·T-205·T-212/212a·T-213a·T-213·T-306·T-307·T-310/310a·T-311·T-507을 동기화한다. T-006의 기존 계정 확보 범위는 사용자 지시로 철회되었으며 조회 성공으로 완료하지 않는다. 게시 정책을 재개하려면 새로운 사용자 지시와 ADR이 필요하다.
