...
2026-09-07 변경: [ADR-014](../adr/014-common-implementation-without-registry-publishing.md)와 T-015가 사용자 지시의 npm/PyPI 미게시·common 단독 구현 순서를 반영했다. [ADR-015](../adr/015-common-shared-systems-scope.md)와 T-016은 common을 독립 실행 시스템이 아닌 위젯·디자인 토큰·공용 코어 로직·로그인 계층을 모으는 저장소로 재확인한다. 로그인 UI와 주입형 인증 프리미티브는 common에 포함하고, 인증 서버·사용자/세션 저장소·운영 비밀·외부 IdP·앱별 정책은 소비자에 남긴다. 전체 대조는 [범위 재점검](common-scope-recheck-2026-09-07.md)이다. 아래 D-01·D-11·D-18·O-5는 이 결정을 연결한다. §3 이후 파일·ADR·Phase·task 표는 최초 설계 스냅샷이며 현재 상태/선행은 [task 원장](../tasks.md)과 [통합 계획](integration-plan.md), ADR 상태는 [단일 색인](../adr/README.md)을 따른다.
...
| O-2 [닫힘·2026-09-08] | ktc·ktdm 루트 GPL 정렬, common §7 추가 허가 없음(L8) | GPL-3.0-or-later 정렬(각 license-only PR 1개, 코드 채택 PR과 분리) | T-021 external evidence(소비자별 license-only PR) → T-45x·T-47x 코드 채택 |
...
| O-24 | airport 14002·`-latest` | 14002 예외; `-latest`는 ktdm. common fixture 포트는 운영 등록하지 않음 | T-014 |
...
| 011 | Python 공통 패키지 구조(extras·3.11 호환·모듈 우선순위·공용 인증 프리미티브 경계·메트릭 접두) | partially superseded by ADR-015 | D-15·D-22 |
...
| T-021 | ktc·ktdm 라이선스 정렬(L8) 결정 반영: 결정 기록·각 저장소 LICENSE-only PR 요청 문서(결정 완료 2026-09-08, 외부 evidence는 소비자별 행) | 없음 | P1 | 문서 | common/ktc/ktdm | 외부 PR 대기 |
...
| T-454 | concierge: ui v0.2 채택(18종 shim·base-ui 1.8·`render` 9줄) | T-021, T-453, T-213 | P2 | e2e 45 | concierge | 외부 선행: T-021 common 결정 완료 + ktc license-only evidence |
...
| T-473 | ktdm: ui 부분 채택(StatStrip·AppErrorPanel·SectionCard; `ops-*` 잔존 허용) | T-021, T-472, T-213 | P3 | vitest | ktdm | 외부 선행: T-021 common 결정 완료 + docker-manager license-only evidence |
