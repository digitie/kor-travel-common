...
| 배포 단위 | `packages/tokens`(npm `@kor-travel/tokens`) · `packages/ui`(npm `@kor-travel/ui`) · `packages/py/kor-travel-common`(PyPI 이름 `kor-travel-common`, import `kortravelcommon`) · `docs/standards/*` 규칙 문서 · `templates/*` · `versions.json` + `tools/*.py` |
...
| O-5 | npm scope `@kor-travel` 확보 | tarball 선행(비차단); 실패 시 `@digitie/kor-travel-*` | T-006 |
