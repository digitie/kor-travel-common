...
- Gate: 문서
- 선행: 없음
- 외부 선행: 사용자 결정 O-1 완료(2026-09-08). 실제 pinvi 반영은 T-420 외부 선행이다.

## 목표

pinvi 전 트랙(T-420·T-421·T-422·T-484, ui 1차 T-212의 pinvi 경로)을 여는 유일한 조건 L6를 결정 기록·요청 문서·검증 가능한 해제 조건으로 정리한다. 외부 LICENSE 반영 전에는 pinvi 유래 코드가 common에 들어오지 않도록 gate를 유지한다.

## 고정 결정

- [설계 브리프](../plan/design-brief.md) D-16(L6 common 결정은 T-020, 실제 반영은 T-420 외부 gate; 반영 전 ui 1차는 airport 소형 부품으로 대체), D-17(B1: pinvi는 L6 전 추출 금지), O-1 채택값.
...
3. 결정·요청 문서와 두 리뷰어 gate가 완료되면 T-020을 DONE으로 기록하고, T-420은 외부 반영 대기 상태로 남긴다. 실제 pinvi merge SHA·LICENSE·공개 문구·패키지 필드 정합 evidence를 확인한 뒤에만 소비·추출 gate를 해제한다.
...
- 이 task는 사용자 결정·결정 기록·요청 문서를 기록했다. pinvi PR·실제 해제 SHA는 T-420 evidence로 연결하며 외부 반영 전까지 G-LIC를 OPEN으로 유지한다.
...
사용자는 “t020 021 관련해서 모두 gplv3로 바꿀꺼야 참고해서 이번에 한번에 닫아”라고 지시했다. 이에 O-1을 **공개 + GPL-3.0-or-later**로 확정하고, common 안에서 결정 기록과 pinvi 요청 문서를 완료했다.

- 요청 문서: [pinvi-license-l6.md](../plan/requests/pinvi-license-l6.md)
- common은 pinvi 저장소를 수정하지 않았고 pinvi의 LICENSE·metadata·README·AGENTS·문서 반영은 T-420 외부 task로 남긴다.
- G-LIC의 pinvi 적용 gate는 외부 PR의 40자리 SHA·LICENSE 첫 줄·자체 검증 evidence를 받을 때까지 OPEN이다.
- T-020의 DONE은 라이선스 결정·요청 문서 gate를 닫았다는 뜻이며, pinvi 코드 추출·소비 gate를 닫았다는 뜻이 아니다.
