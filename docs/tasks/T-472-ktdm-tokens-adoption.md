...
- Gate: 시각 diff(수동)
- 선행: T-470, T-109
- 외부 선행: T-021 common 결정 완료(2026-09-08); ktdm 외부 LICENSE evidence(L8) — 브리프 선행 열에는 없으나 D-16이 L8 전 코드 소비를 금지하므로 tokens 패키지 설치는 L8 후에만
...
- [ ] L8 외부 LICENSE evidence(T-454/T-473 PR 링크·루트 LICENSE) PR 본문 기재; `check_versions` ktdm tokens `OK`.
