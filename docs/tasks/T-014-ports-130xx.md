# T-014 common 포트 `130xx` 로컬 점유 확인·확정 + ktdm `docs/ports.md` sibling(airport 140xx·weather 141xx·common 130xx) 등록 요청 + `-latest` 접미 질의
...
common의 스모크·fixture 서비스가 쓸 포트 대역 `130xx`를 로컬 점유 확인 뒤 확정하고, 포트 정본인 ktdm `docs/ports.md`에 sibling 대역(airport 140xx·weather 141xx·common 130xx)과 airport 14002 예외를 명시 행으로 넣어 달라는 요청과 `-latest` 컨테이너 접미 질의를 문서로 남긴다.

## 고정 결정

- [설계 브리프](../plan/design-brief.md) D-18 포트 절(정본은 ktdm `docs/ports.md`·슬롯 규칙 인용·`130xx`는 점유 확인 후·14002 예외·`-latest`는 ktdm 결정), O-24.
...
2. 슬롯 확정: `13001` consumer-smoke API fixture, `13005` consumer-smoke web fixture, `13010~13099` 임시·E2E(슬롯 규칙 준수). `docs/standards/ci-deploy.md` 포트 절(standards-be 소유)에는 "정본 ktdm `docs/ports.md` 인용 + common 슬롯 3행"만 두도록 링크로 위임.
3. 요청 문서 `docs/plan/requests/docker-manager-ports.md`: sibling 3행(140xx airport·141xx weather·130xx common), airport 14002 예외 등록(대안 14005 이전 비용 요약), `-latest` 접미 유지/제거 질의(ktdm `docker-targets.yml`·geo `docker_app.sh` 결합 비용 인용), 요청 일자·common 커밋.
...
- evidence에 점유 확인 명령·실행 환경·결과(빈 출력 또는 점유 항목)가 있고 조사 7 저장소 grep 결과가 `130xx` 0건이다.
- 요청 문서가 sibling 3행·14002 예외·`-latest` 질의를 포함하고 운영 호스트·IP 값을 담지 않는다(redaction guard 통과).
