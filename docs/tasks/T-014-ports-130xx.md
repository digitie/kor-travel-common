...
4. 답변이 오면 소비자 sibling 요청·14002 예외·`-latest` 질의의 evidence만 갱신하고 이 task를 닫는다. common fixture 포트는 답변과 무관하게 실행 시 주입 원칙을 유지한다.
...
- ktdm이 다른 소비자 대역을 지정하면 요청 문서 evidence만 갱신한다. common 코드에는 운영 포트 상수를 만들지 않고 fixture 설정을 실행 시 주입한다.
