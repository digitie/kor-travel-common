...
- 2026-09-09 post-fix-02/03 수정: 첫 post-fix finding의 근본 원인인 YAML 숫자·timestamp 표기 우회, task 본문 기반 provenance, SHOULD surface/reason 문자열 우회를 닫았다. parser와 Markdown renderer가 Cc/Cf/Zl/Zp를 거부하고, task ID는 `docs/tasks/T-*.md` 파일명에서만 수집하며, S 예외는 정규 surface·긍정적인 `소비하는 외부 계약` 근거·M10/동반 PR을 검증한다. UTF-8 BOM은 문서 시작에서만 허용하고 timezone·sexagesimal timestamp도 plain scalar로 거부한다. focused 시험은 26개, 전체 unittest는 362개이며 문서(532/2583)·plan(106)·SPDX(70)·redaction/secret(683/0)·PR CI 결과가 통과했다. 기능 post-fix-03 candidate `36509d3835a459993ad13b5a10a0340e48178b86`(tree `31c76454764b77a8b8241cc951aa7834f2581570`)에서 상태 evidence를 갱신했으며, 이어지는 docs candidate manifest가 최종 immutable 기준선을 고정한다.
