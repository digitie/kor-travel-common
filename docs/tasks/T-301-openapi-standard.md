...
- 2026-09-09 post-fix-05 수정(역사 기록): base prefix 직후 underscore·부호·short sexagesimal·timezone timestamp를 plain scalar로 거부하고, S 예외의 전역 wildcard(`*`·`/*`·`/**`)와 한국어·영어 부정 근거를 fail-closed로 막았다. renderer가 schema·updated·apps·exceptions top-level을 재검증하고 Markdown cell escape를 적용한다. 당시 candidate는 focused 27개·전체 unittest 364개·문서(536/2583)·plan(106)·SPDX(70)·redaction/secret(687/0) gate를 통과했지만 A/B가 추가 경계를 재현해 BLOCK했다.
- 2026-09-09 post-fix-06 수정(역사 기록): 후보 `d88f2a0446e114541f52de169630cd09e27f429a`와 [manifest](../reviews/adversarial/evidence/2026-09-09-t301-post-fix-06-manifest.md)(SHA-256 `b5b9decc76cecba0e84d7a987443f0549df90174e00ac8d2dd85beb8865d108d`)에서 focused 28개·전체 365개·문서(539/2583)·plan(106)·SPDX(70)·redaction/secret(690/0)·candidate CI `34306605363`/`34306605583`을 고정했다. A/B는 숫자·renderer 경계는 통과했지만 자연어 assertion, 증명 token·task suffix와 stale 문서를 각각 BLOCK했다.
- 2026-09-09 post-fix-07 수정 후보: `EXTERNAL_CONTRACT_RE`를 긍정 assertion의 허용 종결(`이다`·`임`·여는 괄호)로 제한하고, M10·동반 PR·task ID를 Unicode identifier·underscore·점/접미사 경계까지 exact token으로 닫았다. YAML 예약 indicator(`@`, backtick, 단독 `-`·`?`)와 A/B가 재현한 부정·불확정 어휘를 fail-closed로 처리하고, 31개 focused·368개 full unittest를 통과했다. 기능 commit `465ac00` 기준 현재 gate는 문서 542/2584·plan 106·SPDX 70·redaction/secret 693/0이며 docs closure commit과 새 immutable manifest를 만든 뒤 두 reviewer가 같은 후보 SHA를 재검토한다.
- 2026-09-09 post-fix-08 수정 후보: post-fix-07 A/B가 재현한 열린 delimiter·질문형 assertion·공백/underscore/Unicode hyphen 부정·불확정 어휘·combining mark token·YAML `- foo`·`? foo` 경계를 각각 fail-closed로 닫았다. 기능 commit `8959af9`는 외부 계약 assertion을 `계약이다.`·`계약임.`으로 제한하고, M10·동반 PR·task ID를 `L/N/M`·결합문자까지 처리하는 공통 scanner로 전환했으며, 접두·접미 substring assertion과 이중 종결 회귀를 추가하고 concierge S1 정본 사유를 같은 문법으로 맞췄다. focused 36개·full 373개 unittest와 문서 545/2584·plan 106·SPDX 70·redaction/secret 696/0 gate를 통과했다. 문서 closure와 immutable manifest를 고정한 같은 후보 SHA에서 A/B post-fix-08 재검토를 진행한다.
