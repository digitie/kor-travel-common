...
`@kor-travel/ui`(ADR-014 확정)가 컴포넌트 0개 상태에서도 빌드 → `npm pack` → tarball 설치 → Next 16 `next build`(Turbopack·webpack 양쪽)까지 통과하는 배포 골격을 만든다. T-203~T-210은 이 골격 위에 컴포넌트만 추가한다. [브리프](../plan/design-brief.md) D-09가 "소스 확인 전 릴리스 금지"로 지정한 base-ui 미확인 3건(Button `type` 기본, Checkbox hidden input, Toast API)을 이 task에서 확인해 기록한다.
...
- npm/PyPI 게시(사용자 범위 제외)·Renovate 재평가(T-507), 아이콘 전체 세트(필요한 컴포넌트 task에서 추가).
