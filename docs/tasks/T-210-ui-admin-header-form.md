...
- ADR-007 — [ADR 색인](../adr/README.md). [브리프](../plan/design-brief.md) D-13: 접힌 rail 4rem(pinvi 5rem 예외), 셸 전환 1024, inspector rail 22rem은 `xl` 이상 2열, pathname 노출 대신 breadcrumb, nav 접근성 이름은 라벨만, pinvi `AdminPage` → `AdminPageHeader` 수렴(C16). D-20: airport 셸·로그인 소비는 T-035 라우트 분리 후(O-9). D-01: 인증 서비스는 common 범위 밖.
...
- `AdminShell` nav·로그아웃·접힘·RBAC·`NAV_GROUPS`(앱), `LoginForm`/`LoginScreen`(T-211 레지스트리 템플릿 + 앱), pinvi `AdminPage` 어댑터 교체(T-422), weather 셸 교체(T-463), RHF/zod 어댑터, dirty 이탈 경고(O-22 미포함).
...
- 컴포넌트 단위 revert.
- 차단: 헤더에 nav·인증 로직 유입, 폼 부품이 RHF/zod를 import, h1 중복 허용. 하나라도 있으면 T-213 rc를 만들지 않는다.
