...
8. `docs/standards/backend-stack.md` 확정: Python floor/이미지/uv·lock 의무/extras/모듈 우선순위(1차~보류)/메트릭 접두 규칙(D-22)/공용 인증 프리미티브와 앱 소유 경계/provider `python-*-api` SHA는 보고만(D-23). 인증 구현은 T-312에서 수행한다.
