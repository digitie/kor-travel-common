...
- Gate: ci.yml
- 선행: T-471, T-311
- 외부 선행: T-021 common 결정 완료(2026-09-08); ktdm 외부 LICENSE evidence(L8) — 브리프 선행 열에는 없으나 D-16이 L8 전 코드 소비를 금지하므로 common 모듈 import는 L8 후에만
...
- [ ] `check_versions` ktdm Python 행 `OK`; L8 외부 LICENSE evidence(T-473 PR) 본문 기재.
