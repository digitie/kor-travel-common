# T-310a Python 0.1 common 검증 후보 보존

- 상태: BLOCKED
- 우선순위: P1
- Gate: Python 시험·wheel/sdist 설치·후보 보존
- 선행: T-303, T-304, T-305, T-309

## 목표

export·health·time·quality 1차 모듈의 Python 0.1 후보를 보존한다. 실제 map/weather/airport 계약 대조는 T-310에 남는다.
...
4. T-306·T-307은 이 task DONE 뒤 Python 개발 버전을 먼저 변경한다. T-310은 실제 소비자 스모크, T-311은 T-310과 후속 소비자 검증을 계속 요구한다.
...
- T-302의 Python 3.11/3.12/3.13 core·api 설치와 extras 경계, T-303 결정적 export·비변경 check, T-304 health/time·T-305 quality 및 T-309 drift fixture를 실제 후보에서 검증한다. wheel과 sdist를 모두 빌드하고 고지·버전·import를 깨끗한 환경에서 대조한다.
