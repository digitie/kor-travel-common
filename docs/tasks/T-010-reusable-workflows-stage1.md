...
- 로컬 검증: `python -B -X utf8 -m unittest discover -s tests -p 'test_*.py'` 337 tests OK(2026-09-09), `validate_document_links.py` 524 documents/2576 targets 오류 0, `validate_plan.py` 106 tasks 오류 0, `check_spdx.py` 68 files 오류 0, `check_prod_redaction.py --all` 673 files/발견 0, `git diff --check` OK. 두 reviewer 실행 당시 closure 기준은 각각 522/2570·671이었으며, 이 최종 원본 evidence 파일을 추가한 현재 tree의 수치를 별도로 갱신했다.
...
- 기능 후보의 `workflows-selftest`는 [Actions run 34288106079](https://github.com/digitie/kor-travel-common/actions/runs/34288106079), docs·tools·packages·check-versions·secret-scan은 [Actions run 34288105963](https://github.com/digitie/kor-travel-common/actions/runs/34288105963)에서 모두 성공했다. docs-only closure `5b77390`도 [docs·tools·packages·check-versions·secret-scan 34289912536](https://github.com/digitie/kor-travel-common/actions/runs/34289912536)와 [fixture selftest 34289912727](https://github.com/digitie/kor-travel-common/actions/runs/34289912727)에서 모두 성공했다. 상세 리뷰 기준은 [최종 manifest](../reviews/adversarial/evidence/2026-09-09-t010-final-manifest.md)이며 [reviewer A 최종 원본](../reviews/adversarial/evidence/2026-09-09-t010-final-reviewer-a.md)과 [reviewer B 최종 원본](../reviews/adversarial/evidence/2026-09-09-t010-final-reviewer-b.md)이 모두 `PASS`이고 신규·잔여 P0/P1/P2가 없다. 실제 map·weather consumer dispatch, 주간 활성화, 소비자 build/e2e, npm/PyPI 게시·Release는 `NOT_RUN(외부 소비자·사용자 범위; T-010a)`로 유지한다.
