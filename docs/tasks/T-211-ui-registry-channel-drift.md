...
- 이 task에서 확정하는 선택(호스팅은 열림, 기본값): 레지스트리 JSON은 `shadcn build`로 생성해 GitHub Release 자산(`ui-vX.Y.Z` 태그, 파일명 `registry-<item>.json`)으로 올리고 소비자 `components.json` `registries`에 태그 고정 URL을 적는다. 로그인 템플릿은 `fetch("/api/auth/login")` 호출과 `nextPath` prop 골격만 제공하고 세션·CSRF·rate limit은 앱 소유(D-01).
...
- 전면 레지스트리 전환(T-508), 레지스트리로 primitive 배포, 소비자 `components.json` 신설(앱 이관 task), 로그인 인증 로직, `AdminShell` nav 정본, playwright 기준선 파일 자체(T-108).
