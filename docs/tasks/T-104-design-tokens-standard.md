...
색상 톤 규칙의 정본 `docs/standards/design-tokens.md`를 규칙 ID `TK-n`으로 확정한다. 이번 PR에서는 "정본 초안"으로 산출하고(브리프 §7), T-101의 실물 `tokens.css`·`theme.css`와 변수 집합·alias 의미·프로필 값을 대조한 뒤 초안 문구를 지우고 2인 리뷰로 확정한다.
...
1. 머리: 정본 선언 + "정본 초안 — T-101 실물과 대조 후 확정" 1문장(확정 시 제거) + 조사 기준 커밋.
...
- TK-3 표의 변수명 집합이 `rg -o -- '--kt-[a-z0-9-]+' packages/tokens/src/tokens.css | sort -u` 결과와 같다(대조 후).
...
- 확정 시점: 초안 문장이 제거되고 리뷰어 2인(디자인 시스템 · 접근성/대비) verdict `PASS`.

## 검증 명령

```bash
python3 -B -X utf8 tools/validate_document_links.py
rg -o -- '--kt-[a-z0-9-]+' packages/tokens/src/tokens.css | sort -u > /tmp/kt-vars.txt
rg -o -- '`--kt-[a-z0-9-]+`' docs/standards/design-tokens.md | tr -d '`' | sort -u | diff - /tmp/kt-vars.txt && echo "vars match"
...
- 대조표·diff 결과·리뷰 report(`docs/reviews/adversarial/YYYY-MM-DD-design-tokens.md`)를 이 절과 `docs/journal.md`에 남긴다. 초안 단계에서는 대조를 `NOT_RUN(T-101 대기)`로 표기한다.
