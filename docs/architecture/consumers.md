...
|---|---|---|---|---|---|---|---|
| kta | kor-travel-airport | `2bb1111`(main); WIP `99b3f98` `codex/shadcn-ui-foundation` | GPL-3.0 원문 674행(버전 미지정) | 무인증 백업·collector-status 패널 + `/v1/admin/*`(ADR-003 무인증 의도, D-20) | main: 순수 CSS 1,844행, Next 16.3.2, React 19, TS 7.0.2, ESLint 없음; WIP: Tailwind 4.3.3 + shadcn `base-nova` + base-ui 1.8 | FastAPI, RFC7807(`code`·`request_id` 없음, 스펙 422 불일치), export만·CI drift 없음, `uv.lock` CI만(Docker pip) | [kor-travel-airport §8](../survey/inventory/kor-travel-airport.md#8-공통화-후보)·[§9](../survey/inventory/kor-travel-airport.md#9-이-저장소-고유-차이주의점) §1·§3.2·§9, `vm` §6 |
| ktc | kor-travel-concierge(TripMate) | `7945305` | MIT(정렬 대상 L8) | admin | Next 16.2.7, base-ui 1.5 `base-nova` 18종(map 계보 부분집합), `--ktc-*` 125회, `@config`, maplibre-gl 6.0 | FastAPI, `/api/v1`, `{detail}`, `requirements.txt` 4벌 하한만(`mcp<2` 사고 2026-09-04), CI 없음, 라우터 단일 파일 3,759줄 | [kor-travel-concierge §8](../survey/inventory/kor-travel-concierge.md#8-공통화-후보)·[§9](../survey/inventory/kor-travel-concierge.md#9-이-저장소-고유-차이주의점), `vm` §5.2 |
| ktdm | kor-travel-docker-manager(ktdctl) | `862562d` | MIT(L8) | admin | Next 14.2.35, React 18.3.1, ESLint 8, Tailwind 4.3.1 `@theme` OKLCH, `ops-*` CSS, shadcn 미도입, Node 20 CI | FastAPI, `/api/v1`, `{detail}`, SQLite, Poetry(lock 미커밋), systemd+venv(컨테이너 아님), 포트 정본 `docs/ports.md` | [kor-travel-docker-manager §8](../survey/inventory/kor-travel-docker-manager.md#8-공통화-후보)·[§9](../survey/inventory/kor-travel-docker-manager.md#9-이-저장소-고유-차이주의점) §2·§6 |
| geo | kor-travel-geo(+kor-travel-geo-ui) | `1d9d74d` | GPL-3.0-only(O-20) | admin ui | Next 16.2.12, React 18.3.1(ADR-019), radix-ui 1.6 `radix-nova`, `@config` 잔존, Tailwind 4.3.1, openapi-typescript 7.13, Node 20 CI | FastAPI, `/v1/healthz`·`/v1/readyz`, envelope 3종(VWorld/v2/legacy), 검증 400, ruff+mypy strict+import-linter, Python lock 없음 | [kor-travel-geo §8](../survey/inventory/kor-travel-geo.md#8-공통화-후보)·[§9](../survey/inventory/kor-travel-geo.md#9-이-저장소-고유-차이주의점) §3.1·§9 |
| map | kor-travel-map(npm workspaces) | `c494e227` | GPL(25행 요약본, L9 전문 복원) | admin(UI 정본) + api + dagster | Next 16.2.12 exact, base-ui 1.6 `base-nova`(CLI 미설치), npm 12.0.1 exact + `verify-*.mjs`, Playwright 1.60 exact, e2e 30·vitest 42 | FastAPI 원형(`ProblemDetail`·`{data,meta}`·profile export `--check`), `starlette<1.0`·`alembic<1.20`, Python lock 없음 | [kor-travel-map §8](../survey/inventory/kor-travel-map.md#8-공통화-후보)·[§9](../survey/inventory/kor-travel-map.md#9-이-저장소-고유-차이주의점) §9, `oa` §3 |
| wx | kor-travel-weather | `6003da9` | GPL-3.0 | admin | Tailwind 없음, 2,495행 CSS(map 값 복사, hue navy, map 어휘 294회, `--rail` 17rem), Next 15.5.24, Vitest 3.2, Node 20 CI, react-query 선언만 | FastAPI, `Problem` 동형, `/health`·`/version`, `uv.lock` CI·Docker `--locked`, `python-airkorea-api` 벤더링 | [kor-travel-weather §8](../survey/inventory/kor-travel-weather.md#8-공통화-후보)·[§9](../survey/inventory/kor-travel-weather.md#9-이-저장소-고유-차이주의점) §3.1·§8·§9.1 |
| pinvi | PinVi(apps/web·mobile·api·etl) | `9af25e5`(shallow clone) | 루트 `LICENSE` 없음(README "비공개" vs AGENTS "공개", `apps/api` MIT) → L6 | admin(web) | admin: KTM 이식 28 primitive + base-ui 1.8 overlay만, `--color-admin-*`, `@config` v3 preset, webpack 강제(ADR-066), `AdminTable` 어댑터 35~36쪽, e2e 56·vitest 27 | FastAPI, `{error:{}}`, argon2id+JWT(10분)+RBAC, `uv.lock` 미소비(`pip install -e`), etl `python-kasi-api@main` | [pinvi §8](../survey/inventory/pinvi.md#8-공통화-후보)·[§9](../survey/inventory/pinvi.md#9-이-저장소-고유-차이주의점) §3.1·§3.2, `lic` §3.6 |
