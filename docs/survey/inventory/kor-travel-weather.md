...
- pre-commit: 없음.
- compose: `compose.yaml`(7 서비스: `db` postgres:16-alpine shm 1g, `api`, `dagster`, `dagster-gateway` nginx Basic Auth, `migrate` one-shot `alembic upgrade head`, `prometheus` v3.5.0, `web`), `deploy/compose.n150.yaml`(api/dagster-gateway/web 포트를 `192.168.1.14`로 `!override`, `WEATHER_UI_TRUST_PROXY=true`, 공개 origin).
...
- prod(n150) 규약(`deploy/n150.md`): 호스트 `digitie@192.168.1.14`, 경로 `~/kor-travel-weather`, `.env` 모드 600, 배포는 `git pull --ff-only` + `docker compose --env-file .env -f compose.yaml -f deploy/compose.n150.yaml up -d --build`. 도메인 `https://weather-api.digitie.mywire.org`, `https://weather-dagster.digitie.mywire.org`(Basic Auth), `https://weather.digitie.mywire.org`. HAProxy upstream 매핑 명시. Prometheus 14104는 공개하지 않음.
