...
- prod(n150) 규약: `scripts/deploy_app.py` 노드 `n150=deploy@n150.local,linux/amd64`, `odroid=…,linux/arm64`; 공개 도메인은 추적 파일에 두지 않음(`.env.prod.example` placeholder만, `KTG_DAGSTER_PUBLIC_URL`은 공개값으로 커밋 가능 명시). 배포·검증 반복 실수는 gitignored `docs/deploy-runbook.local.md`.
