...
| Task | 상태 | 우선순위 | 제목 | 선행 |
|---|---|---|---|---|
| [T-102](tasks/T-102-map-vocabulary-shim.md) | READY | P0 | 레거시 어휘 별칭 shim `aliases/map-vocabulary.css`(map·weather·geo 공통 이름 → `--kt-*`) + weather `--rail`·font 오버라이드 예제 + 별칭 충돌 검사 스크립트 | T-101 |
