...
- 작업: [T-016](tasks/T-016-common-shared-systems-scope.md), IN_PROGRESS. 사용자 지시와 기존 ADR·architecture·standards·task의 인증 범위 충돌을 ADR-015와 범위 재점검 문서로 해소하고, 두 reviewer의 적대적 리뷰를 거친 뒤 T-011을 BLOCKED에서 READY로 전환한다.
...
[T-016](tasks/T-016-common-shared-systems-scope.md)의 ADR·범위 재점검·정본 문구부터 대조한다. 이후 [T-011](tasks/T-011-consumer-manifest-schema.md)의 소비자 매니페스트 스키마·validator 계약으로 순차 진행한다. 명령 사다리는 [개발 환경](dev-environment.md#6-검증-명령-사다리), 리뷰는 [agent workflow](runbooks/agent-workflow.md)를 따른다.
