...
- 작업: [T-101](tasks/T-101-tokens-package.md), READY. T-011의 매니페스트·입력 경계가 PR #10 merge 후 main CI까지 닫히면 디자인 토큰 정본 구현을 시작한다.
...
[T-101](tasks/T-101-tokens-package.md)의 토큰 정본·생성물 계약을 다음 시작 파일로 둔다. T-011은 최종 리뷰·CI를 통과했지만 PR #10 merge와 main CI 확인을 먼저 닫는다. 명령 사다리는 [개발 환경](dev-environment.md#6-검증-명령-사다리), 리뷰는 [agent workflow](runbooks/agent-workflow.md)를 따른다.
