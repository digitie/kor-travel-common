...
- 작업: [T-104](tasks/T-104-design-tokens-standard.md), IN_PROGRESS(P0). T-101·T-102에서 구현한 tokens 실물과 디자인 토큰 표준 문서를 대조해 TK-n 규칙을 확정하고 두 적대적 리뷰를 진행한다. T-103은 독립 READY(P1)로 유지한다.
...
[T-104](tasks/T-104-design-tokens-standard.md)의 표준 문서와 T-101·T-102의 tokens 실물·review evidence를 다음 시작 자료로 둔다. 명령 사다리는 [개발 환경](dev-environment.md#6-검증-명령-사다리), 리뷰는 [agent workflow](runbooks/agent-workflow.md)을 따른다.
...
- 외부 선행: T-420(pinvi LICENSE 반영), T-454/T-473(concierge·docker-manager LICENSE evidence), airport 병합 후 현재 기준선·미해결 CI 재확인(T-430), geo React 승인(T-443), pinvi mobile 명시 예외(O-8). T-020·T-021의 common 결정은 완료됐지만 외부 반영은 승인 evidence가 아니다.
