...
[PR #1](https://github.com/digitie/kor-travel-common/pull/1)의 계획·문서와 [PR #2](https://github.com/digitie/kor-travel-common/pull/2)의 T-003 고지·출처·SPDX 구현을 main에 squash merge했다. [PR #3](https://github.com/digitie/kor-travel-common/pull/3)의 T-005는 f050997에서 두 독립 reviewer PASS, 최초 9개 finding 모두 FIXED다. 완료 기록 CI 34064140128 성공 후 PR #3을 659aa6d로 병합했다. T-015의 npm/PyPI 미게시·common 구현 선행 정리와 T-006의 범위 변경 기록을 [PR #4](https://github.com/digitie/kor-travel-common/pull/4)에 반영했다. 최종 candidate 95c139f는 두 reviewer PASS·누적 9개 finding FIXED이며 CI 34066138272가 성공했다. 현재 완료 11개·열린 91개다. T-009 리뷰에서 workflow 정적 검사 범위의 누락을 T-005c로 명시했다.
...
T-009는 [PR #5](https://github.com/digitie/kor-travel-common/pull/5)의 f15072f에서 두 reviewer PASS·원 8 finding 모두 FIXED다. Windows/WSL 각각 135 tests·skip 0, candidate PR 및 동일 tree의 빈 검증 commit PR/release push가 필수 5 check 모두 success다. [최종 리뷰](reviews/adversarial/2026-09-07-t009-post-fix.md)와 [CI evidence](tasks/T-009-ci-hardening.md#evidence)에 기록했다. 완료 기록 반영과 PR 병합·main 확인을 진행한다.
...
[T-005a](tasks/T-005a-check-versions-uv-lock.md), [버전 규약](standards/versions.md)의 Python lockfile 계약과 `tools/check_versions.py`의 기존 uv 파서부터 대조한다. 명령 사다리는 [개발 환경](dev-environment.md#6-검증-명령-사다리), 리뷰는 [agent workflow](runbooks/agent-workflow.md)를 따른다.
