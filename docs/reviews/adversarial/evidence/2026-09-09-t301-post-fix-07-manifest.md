# T-301 post-fix-07 적대적 리뷰 입력 manifest
...
- post-fix-07 tree: `c724361a4eac4c2b3697431e0d9a2baeb74a62ce`
- PR: `#22` (draft)
- 작성일: 2026-09-09
...
- 기준 명령: `python -B -X utf8 tools/openapi_exceptions.py --check`; `python -B -X utf8 -m unittest discover -s tests -p 'test_openapi_exceptions.py' -v`; `python -B -X utf8 -m unittest discover -s tests -p 'test_*.py' -v`; 문서 link/plan/SPDX/redaction/secret scan; `git diff --check`; `gh run list --commit 1705a6a10954120b74e59d8611a520e7d9970605`; PR CI checks
- 현재 candidate evidence: focused 31 tests, full 368 tests, document links 542/2584, plan 106, SPDX 70, redaction/secret 693/0, OpenAPI 39건·Markdown 54줄. focused/full은 기능 commit `465ac00`에서 실행했고 docs-only closure candidate에서는 focused·문서 gate를 재실행했으며, reviewer가 final candidate에서 full을 재실행한다.
- candidate CI: docs run `34308915146`, workflows selftest `34308915282`; 두 run 모두 candidate SHA를 가리키며 완료·성공했다.
- 입력 고정: reviewer A/B는 이 manifest와 `post-fix-07 후보` SHA/tree를 사용하고 서로의 결과·통합 report를 읽지 않는다. reviewer는 후보 파일을 수정하지 않고 별도 detached worktree에서 검토한다. post-fix-06의 P1/P2인 자연어 assertion·부정/불확정 어휘, M10·동반 PR·task suffix, YAML 예약 indicator와 stale evidence가 실제로 닫혔는지 새 기준선에서 재현한다.
