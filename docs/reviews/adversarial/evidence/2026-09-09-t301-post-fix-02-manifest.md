# T-301 post-fix-02 적대적 리뷰 입력 manifest

- Review ID: `T301-20260909-post-fix-02`
...
- post-fix-02 tree: `815332f620177536a7306bac44857ea4ea227a6b`
- PR: `#22` (draft)
- 작성일: 2026-09-09
...
- 리뷰 범위: 후보 전체 diff와 OpenAPI 정본·ADR-009/D-14·T-301 계약, parser fail-closed 경계, YAML 숫자·timestamp scalar, task 파일명 provenance, SHOULD 외부 계약 증명·surface 정규화, Unicode 제어·format 문자, 생성 Markdown 안전성, 날짜·원자 출력·alias, 회귀 시험
...
- 입력 고정: reviewer A/B는 이 manifest와 `post-fix-02 후보` SHA/tree를 사용하고 서로의 결과·통합 report를 읽지 않는다. reviewer는 후보 파일을 수정하지 않고 별도 detached worktree에서 검토한다. post-fix-01의 BLOCK finding이 모두 닫혔는지 새 기준선에서 재현한다.
