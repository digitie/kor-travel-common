...
| `validate_manifest.py` | `consumer-manifest.v1` 필수 필드·미지 필드·`enforce` 금지·lockfile 종류·상대 경로·예외 날짜·`versions.json consumers` 정식 repo key를 strict 검사(exit 0/1) | `python3 -B -X utf8 tools/validate_manifest.py <kor-travel-common.lock.json>` |
...
매니페스트 기반 버전 검사는 소비자 저장소 루트와 매니페스트 경로를 함께 준다. `lockfiles[].path`와 `.github/workflows`는 저장소 루트에서 해석한다.
