...
| `openapi_exceptions.py` | OpenAPI 예외 YAML의 7키·규칙 ID·날짜·즉시 MUST sunset을 검증하고 사람이 읽는 Markdown 표를 생성. PyYAML 없이 flat mapping/list 부분집합만 허용하며 `--check`로 생성물 drift를 실패 처리 | `python3 -B -X utf8 tools/openapi_exceptions.py --write` / `python3 -B -X utf8 tools/openapi_exceptions.py --check` |
