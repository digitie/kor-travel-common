...
이 문서는 `kor-travel-common` 모든 작업에 적용되는 짧은 규칙의 정본이다. 확정 task: T-001(초안, 2인 리뷰 전) · 마지막 갱신: 2026-09-06. 결정 근거는 [설계 브리프](docs/plan/design-brief.md)의 결정 레지스터(D-xx)이며, 이 문서는 그 결정을 규칙 문장으로만 옮긴다.
